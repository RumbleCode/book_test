...
//기존의 http서버에 피기백(piggyback)방식으로 SocketIO서버를 시작
 exports.listen = function(server){
    ...
    io.sockets.on('connection', function(socket){ // 각 연결을 어떻게 처리해야 할지 정의
        ...
        handleMessageBroadcasting(socket, nickNames, namesUsed); //사용자의 메시지,닉네임변경,채팅방 생성이나 변경에 관한 처리.
...
function joinRoom(socket, room){
    ...
    if(usersInRoom.length > 1){
        ...
        var usersInRoomSummery = 'Users current in' + room + ': ';

            for(var peoplelist in usersInRoom){
                var userSocketId = usersInRoom[peoplelist].id;
